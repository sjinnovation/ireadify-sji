...
import { updateFavoriteBook } from '../../api/mutations'
import { listFavoriteBooks } from '../../api/queries'
import { useRouter } from 'next/router'
import { isDeleted, bookType, userRole } from "../../config/bootstarp"
import _ from 'lodash';
import Link from 'next/link'
import Skeleton from '../library/skeletonBooks';
import RecentBooks from '../../components/mybooks/RecentBooks';
...
const MyBooks = () => {
   ...
   let fetchBook = async () => {
      ...
      var allFavBooks = allFavoriteBooks.data.listFavoriteBooks.items.filter(el => {
         return el.books.isDeleted == isDeleted.no;
     });;
                     ...
                     <div className="sm:w-full sm:px-5">
                        <div className="justify-items-center">
                           <div data-controller="tabs" data-tabs-active-tab="-mb-px border-l border-t border-r rounded-t">
                              <ul className="list-reset flex border-b-2 md:mx-6">
                                                            ...
                                                            <div className="relative w-48 h-60 mb-20 bg-green-400 shadow-inner">
                                                               <Link href={`/book/${item.bookId}`}>
                                                                  <div className="inner-shadow img-book">
                                                                     <img className="cursor-pointer bg-gray-900 bg-opacity-10" src={item.books.image} alt={item.books.title} />
                                                                  </div>
                                                               </Link>
                                                               <div className="flex absolute justify-between p-2 shadow-inner top-0 w-full h-20 bg-ratings">
                                                                  {
                                                                     item.isLiked === true ?
                                                                        <div className="flex cursor-pointer" onClick={() => onFavoriteClick(item.id)}>
                                                               ...
                                                               <div className="flex flex-col truncate">
                                                                  ...
                                                                  <Link href={`/book/${item.bookId}`}>
                                                                     <h5 className="cursor-pointer flex font-sans text-4xl font-semibold text-gray-700">{item.books.title}</h5>
                                                                  </Link>
                                                                  <Link href={`/author/${item.books.authorId}`}>
                                                                     <p className="cursor-pointer flex font-serif text-small1 text-gray-500">{item.books.authorName}</p>
                                                                  </Link>
