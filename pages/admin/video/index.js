...
import { searchVideos as SearchVideos } from '../../../api/queries'
...
const index = () => {
    ...
    let fetchVideo = async (keyword)  => {
        ...
        let  video  = await API.graphql(graphqlOperation( SearchVideos, {
            sort: {direction: 'desc', field: 'createdAt'},
            ...condition 
        }))
        setVideo(video.data.searchVideos.items)
        setLoading(false)
      }
    ...
    const deleteVideo = async (id , title) => {
        var isDelete = confirm("Are you sure to delete " + title + " video ? ");
        if (isDelete == true) {
            const inputData = {
                id: id,
            ...
            const filteredTodos = videos.filter(item => item.id !== id)
            setVideo(filteredTodos)
        } 
    }
                ...
                <div className="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
                    <div className="p-4">
                        <div className="bg-white flex border-solid border-black items-center rounded-full shadow-xl">
                            <input className="rounded-l-full w-full py-4 px-6 text-gray-700 leading-tight focus:outline-none" name="keyword" value={keywordValue} onChange={handleSearchText} type="text" placeholder="Search by Title" />
                            {keywordValue === "" ?
                                <div className="p-4">
                                ...
                                <div className="p-4 flex flex-1">
                                    <button onClick={searchVideo} className="bg-blue-500 text-white  py-2 px-4 mr-1 hover:bg-blue-400 focus:outline-none  flex items-center justify-center">
                                        Search
                                    </button>
                                    {
                                        searchCancelButton ? 
                                    ...
                                    <td className="border px-4 py-2 ">
                                        <div className="flex flex-1 justify-center">
                                            <span className="text-red-500 block w-4 cursor-pointer mx-2 ">
                                                <Link href={`/admin/video/edit/${item.id}`}>
                                            ...
                                            <span className="text-red-500 block w-4 cursor-pointer mx-2" onClick={() => deleteVideo(item.id, item.title)}>
                                                <svg  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
