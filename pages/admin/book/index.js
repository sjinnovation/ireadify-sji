...
import { searchBooks as SearchBooks } from '../../../api/queries'
...
const index = () => {
    ...
    let fetchBook = async (keyword)  => {
        ...
        let  books  = await API.graphql(graphqlOperation( SearchBooks, {
            sort: {direction: 'desc', field: 'createdAt'},
            ...condition 
        }))
        setBooks(books.data.searchBooks.items)
        setLoading(false)
      }
    ...
    const deleteBook = async (id , title) => {
        var isDelete = confirm("Are you sure to delete " + title + " book ? ");
        if (isDelete == true) {
            const inputData = {
                id: id,
             ...
             const temporaryArray = books.filter(item => item.id !== id)
                ...
                <div className="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
                    <div className="p-4">
                        <div className="bg-white flex border-solid border-black items-center rounded-full shadow-xl">
                            <input className="rounded-l-full w-full py-4 px-6 text-gray-700 leading-tight focus:outline-none" name="keyword" value={keywordValue} onChange={handleSearchText} type="text" placeholder="Search by Book" />
                            {keywordValue === "" ?
                                <div className="p-4">
                                ...
                                <div className="p-4 flex flex-1">
                                    <button onClick={searchBooks} className="bg-blue-500 text-white  py-2 px-4 mr-1 hover:bg-blue-400 focus:outline-none  flex items-center justify-center">
                                        Search
                                    </button>
                                    {
                                        searchCancelButton ? 
                        ...
                        <table className="w-full rounded">
                            ...
                            <tbody>
                            {
                                ...
                                <tr className="text-center" key={item.id} >
                                    ...
                                    <td className="border px-4 py-2">{item.bookType}</td>
                                    <td className="border px-4 py-2">{item.authors.name}</td>
                                    ...
                                    <td className="border px-4 py-2 ">
                                        <div className="flex flex-1 justify-center">
                                            <span className="text-red-500 block w-4 cursor-pointer mx-2 ">
                                                <Link href={`/admin/book/edit/${item.id}`}>
                                            ...
                                            <span className="text-red-500 block w-4 cursor-pointer mx-2" onClick={() => deleteBook(item.id, item.title)}>
                                                <svg  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
