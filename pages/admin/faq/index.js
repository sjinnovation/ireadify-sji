...
import { searchFaqs as SearchFaqs } from '../../../api/faq/queries'
...
const index = () => {
        ...
        try {
            ...
            if (keyword) {
                condition = {
                    filter: {
                        isDeleted: { eq: isDeleted.no },
                        question: { matchPhrasePrefix: keyword }
                    }
                }
                setSearchCancelButton(true)
            }
            let faq = await API.graphql(graphqlOperation(SearchFaqs, {
                sort: { direction: 'asc', field: 'topic' },
                ...condition
            }))
            setFaq(faq.data.searchFaqs.items)
            setLoading(false)
        }
        catch
        (error) {
    ...
    let searchFAQ = async () => {
        fetchFaq(keywordValue)
        let Query = "?keyword=" + keywordValue;
        var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + Query;
        window.history.pushState({ path: newurl }, '', newurl);
    }

    const deleteFaqs = async (id, question) => {
        var isDelete = confirm("Are you sure to delete '" + question + "' FAQ?");
        if (isDelete == true) {
            const inputData = {
                id: id,
            ...
            const filteredFaq = faq.filter(item => item.id !== id)
            setFaq(filteredFaq)
        }
    }
                ...
                <div className="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
                        ...
                        <div className="bg-white flex border-solid border-black items-center rounded-full shadow-xl">
                            ...
                            {keywordValue === "" ?
                                ...
                                <div className="p-4 flex flex-1">
                                    <button onClick={searchFAQ} className="bg-blue-500 text-white  py-2 px-4 mr-1 hover:bg-blue-400 focus:outline-none  flex items-center justify-center">
                                        Search
                                    </button>
                                    {
                                        searchCancelButton ?
                    ...
                    <Link href="/admin/faq/add">
                        <a className="flex align-baseline content-end justify-items-end m-1 p-1">
                            <button className="bg-green-500 text-white  py-2 px-4  mr-1 focus:outline-none  flex items-center justify-center">                                Add
                            </button>
                        </a>
                    </Link>
                        {
                                        ...
                                        <tbody>
                                            {
                                                faq.map((item) =>
                                                    <tr className="text-center" key={item.id}>
                                                        ...
                                                        <td className="border px-4 py-2">{moment(item.createdAt).format('L')}<br></br>{moment(item.createdAt).format('LTS')}</td>                                                    <td className="border px-4 py-2 ">
                                                            <div className="flex flex-1 justify-center">
                                                                ...
                                                                <span title="Delete" className="text-red-500 block w-4 cursor-pointer mx-2" onClick={() => deleteFaqs(item.id, item.question)}>
                                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
