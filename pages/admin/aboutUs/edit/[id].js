...
import { getAbout as GetAboutUs } from '../../../../api/aboutUs/queries';
...
const Edit = () => {
    ...
    const { data: aboutUs, error } = useSWR(
        [GetAboutUs, id],
        fetcher
    )
    ...
    const onSubmit = async (data) => {
        try {
            ...
            let url = ""
            if (file) {
                let key = `aboutUsImages/${uuidv4()}${fileName}`;
                ...
                url = aboutUs.data.getAbout.image
            }
            ...
            const aboutUsList = await API.graphql(graphqlOperation(updateAbout, { input: inputData }))
    ...
    return (
        ...
        <Layout>
            <SuccessAlert show={successAlert} fade={fade} message={"About Us details updated."} hideAlert={onAlertButtonClose} animateAlert={onAlertAnimationEnd} />
                                ...
                                <div>
                                    <div className=" mt-4 mb-4 flex justify-center items-center">
                                        <div className="group w-48 h-48 overflow-hidden shadow-inner text-center bg-purple table cursor-pointer">
                                            <img src={aboutUs.data.getAbout.image} alt="image" className="object-cover object-center w-full h-full visible group-hover:hidden" />
                                        </div>
                                    </div>
                                    <div className="p-3">
                                        <form className="w-full" onSubmit={handleSubmit(onSubmit)}>
                                            <div className="flex flex-wrap -mx-3 mb-6">
                                                <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                                                    ...
                                                    <input defaultValue={aboutUs.data.getAbout.order} ref={register({ required: true, min: 1, validate: (value) => { return !!value.trim() } })} name="order" className="appearance-none block w-full bg-grey-200 text-grey-darker border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-grey"
                                            ...
                                            <div className="w-full mb-6">
                                                ...
                                                <input defaultValue={aboutUs.data.getAbout.title} ref={register({ required: true, minLength: 3, validate: (value) => { return !!value.trim() } })} name="title" className="appearance-none block w-full bg-grey-200 text-grey-darker border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-grey"
