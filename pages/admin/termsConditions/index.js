...
import { searchTermsConditionss as SearchTermsConditions } from '../../../api/queries'
...
const index = () => {
    ...
    const [termsConditions, setPrivacy] = useState()
    ...
    useEffect(() => {
        ...
        fetchTermsConditions(keyword)
    }, [keyword])

    let fetchTermsConditions = async (keyword) => {
        try {
            setLoading(true)
            var condition = {
                filter: {
            ...
            let termsConditionsList = await API.graphql(graphqlOperation(SearchTermsConditions, {
                sort: { direction: 'asc', field: 'order' },
                ...condition
            }))
            setPrivacy(termsConditionsList.data.searchTermsConditionss.items)
            setLoading(false)
        }
        catch
        (error) {
    ...
    let searchPrivacy = async () => {
        fetchTermsConditions(keywordValue)
        let Query = "?keyword=" + keywordValue;
        var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + Query;
        window.history.pushState({ path: newurl }, '', newurl);
    }

    const deletePrivacy = async (id, title) => {
        var isDelete = confirm("Are you sure to delete '" + title + "' Terms?");
        if (isDelete == true) {
            const inputData = {
                id: id,
                isDeleted: isDeleted.yes
            }
            await API.graphql(graphqlOperation(updateTermsConditions, { input: inputData }))
            const filteredTerms = termsConditions.filter(item => item.id !== id)
            setPrivacy(filteredTerms)
        }
    }
        ...
        <Layout>
            <div className="flex flex-1  flex-col md:flex-row lg:flex-row mx-2">
                <div className="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
                        ...
                        <div className="bg-white flex border-solid border-black items-center rounded-full shadow-xl">
                            ...
                            {keywordValue === "" ?
                                ...
                                <div className="p-4 flex flex-1">
                                    <button onClick={searchPrivacy} className="bg-blue-500 text-white  py-2 px-4 mr-1 hover:bg-blue-400 focus:outline-none  flex items-center justify-center">
                                        Search
                                    </button>
                                    {
                                        searchCancelButton ?
                    ...
                    <div className="p-3">
                        <Link href="/admin/termsConditions/add">
                                                        ...
                                                        <td className="border px-4 py-2 ">
                                                            <div className="flex flex-1 justify-center">
                                                                <span title="Edit" className="text-red-500 block w-4 cursor-pointer mx-2 ">
                                                                    <Link href={`/admin/termsConditions/edit/${item.id}`}>
                                                                ...
                                                                <span title="Delete" className="text-red-500 block w-4 cursor-pointer mx-2" onClick={() => deletePrivacy(item.id, item.title)}>
                                                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
