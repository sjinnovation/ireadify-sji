...
import { updatePrivacyPolicy } from '../../../../api/privacy/mutations'
import useSWR from 'swr'
import { getPrivacyPolicy as GetPrivacyPolicy } from '../../../../api/privacy/queries';
...
const Edit = () => {
    ...
    const { data: privacy, error } = useSWR(
        [GetPrivacyPolicy, id],
        fetcher
    )
    ...
    const onSubmit = async (data) => {
        try {
            ...
            await API.graphql(graphqlOperation(updatePrivacyPolicy, { input: inputData }))
                                        ...
                                        <form className="w-full" onSubmit={handleSubmit(onSubmit)}>
                                            <div className="flex flex-wrap -mx-3 mb-6">
                                                ...
                                                <div className="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                                                    ...
                                                    <input defaultValue={privacy.data.getPrivacyPolicy.title} ref={register({ required: true, minLength: 3, validate: (value) => { return !!value.trim() } })} name="title" className="appearance-none block w-full bg-grey-200 text-grey-darker border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-grey"
                                            ...
                                            <div className="flex flex-wrap -mx-3 mb-6">
                                                <div className="w-full px-3">
                                                    <label className="block uppercase tracking-wide text-grey-darker text-xs font-light mb-1">
                                                     ...
                                                     <CKEditor
                                                            data={ckContent !== false ? ckContent : privacy.data.getPrivacyPolicy.content }
                                                            onChange={editorChange}
                                                    />
                                                    <textarea style={{display: "none"}}  value={ckContent !== false ? ckContent : privacy.data.getPrivacyPolicy.content}  ref={register({ required: true, minLength: 3, validate: (value) => { return !!value.trim() } })} name="content" className="appearance-none block w-full bg-grey-200 text-grey-darker border border-grey-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-grey"
