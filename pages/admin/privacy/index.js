...
import { searchPrivacyPolicys as SearchPrivacyPolicys } from '../../../api/queries'
...
const index = () => {
    ...
    useEffect(() => {
        ...
        fetchPrivacy(keyword)
    }, [keyword])

    let fetchPrivacy = async (keyword) => {
        try {
            setLoading(true)
            var condition = {
                filter: {
            ...
            let privacyList = await API.graphql(graphqlOperation(SearchPrivacyPolicys, {
                sort: { direction: 'asc', field: 'order' },
                ...condition
            }))
            setPrivacy(privacyList.data.searchPrivacyPolicys.items)
            setLoading(false)
        }
        catch
        (error) {
    ...
    const deletePrivacy = async (id, title) => {
        var isDelete = confirm("Are you sure to delete '" + title + "' Privacy?");
        if (isDelete == true) {
            const inputData = {
                id: id,
            ...
            const filteredPrivacy = privacy.filter(item => item.id !== id)
            setPrivacy(filteredPrivacy)
        }
    }
                                                        ...
                                                        <td className="border px-4 py-2 ">
                                                            <div className="flex flex-1 justify-center">
                                                                <span title="Edit" className="text-red-500 block w-4 cursor-pointer mx-2 ">
                                                                    <Link href={`/admin/privacy/edit/${item.id}`}>
