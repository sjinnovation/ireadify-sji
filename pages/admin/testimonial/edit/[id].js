...
import { updateTestimonial } from '../../../../api/testimonial/mutations'
import useSWR from 'swr'
import { getTestimonial as GetTestimonial } from '../../../../api/testimonial/queries';
...
const Edit = () => {
    ...
    const { data: testimonial, error } = useSWR(
        [GetTestimonial, id],
        fetcher
    )
    ...
    const onSubmit = async (data) => {
        try {
            ...
            let url = ""
            if (file) {
                let key = `clientTestimonialImages/${uuidv4()}${fileName}`;
                ...
                url = testimonial.data.getTestimonial.image
            }
            const inputData = {
                id: id,
            ...
            await API.graphql(graphqlOperation(updateTestimonial, { input: inputData }))
            testimonial.data.getTestimonial.image = url
    ...
    return (
        ...
        <Layout>
            <SuccessAlert show={successAlert} fade={fade} message={"Author details are Saved."} hideAlert={onAlertButtonClose} animateAlert={onAlertAnimationEnd} />
                                ...
                                <div>
                                    <div className=" mt-4 mb-4 flex justify-center items-center">
                                        <div className="group w-48 h-48 overflow-hidden shadow-inner text-center bg-purple table cursor-pointer">
                                            <img src={testimonial.data.getTestimonial.image} alt={testimonial.data.getTestimonial.name} className="object-cover object-center w-full h-full visible group-hover:hidden" />
                                        </div>
                                    </div>
                                    <div className="p-3">
                                        <form className="w-full" onSubmit={handleSubmit(onSubmit)}>
                                            <div className="flex flex-wrap -mx-3 mb-6">
                                                <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                                                    ...
                                                    <input defaultValue={testimonial.data.getTestimonial.name} ref={register({ required: true, maxLength: 50, minLength: 3, validate: (value) => { return !!value.trim() } })} name="name" className="appearance-none block w-full bg-grey-200 text-grey-darker border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-grey"
                                                ...
                                                <div className="w-full md:w-1/3 px-3 mb-6 md:mb-0">
                                                    ...
                                                    <input defaultValue={testimonial.data.getTestimonial.title} ref={register({ required: true, maxLength: 50, minLength: 3, validate: (value) => { return !!value.trim() } })} name="title" className="appearance-none block w-full bg-grey-200 text-grey-darker border border-grey-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-grey"
                                            ...
                                            <div className="flex flex-wrap -mx-3 mb-6">
                                                <div className="w-full px-3">
                                                    ...
                                                    <textarea defaultValue={testimonial.data.getTestimonial.testimonial} ref={register({ required: true, minLength: 10, validate: (value) => { return !!value.trim() } })} name="testimonial" className="appearance-none block w-full bg-grey-200 text-grey-darker border border-grey-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-grey"
